...
import { ColorPalette, ExtCommands, themeColors, EXT_ROOT } from './constants';
...
export class Timer {
  ...
  get color(): Partial<ColorPalette> {
    return {
      background: this._bar.backgroundColor,
      foreground: this._bar.color
    };
  }
  set color(c: ColorPalette) {
    this._bar.backgroundColor = c.background;
    this._bar.color = c.foreground;
  }
  ...
  private setColor() {
    if (this._bar.command !== ExtCommands.ROTATE_ACTIVE_USERS) {
      this.color = themeColors.def;
    } else if (this.color.background === themeColors.neg.background) {
      this.color = themeColors.pos;
    } else {
      this.color = themeColors.neg;
    }
